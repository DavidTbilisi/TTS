...
class TestUltraFast:
    ...
    def test_determine_strategy_short_text(self):
        ...
        strategy, chunks, workers = determine_strategy("Short text", language="en", max_workers=4)
    ...
    def test_determine_strategy_medium_text(self):
        ...
        strategy, chunks, workers = determine_strategy(text, language="en", max_workers=4)
    ...
    def test_determine_strategy_long_text(self):
        ...
        strategy, chunks, workers = determine_strategy(text, language="en", max_workers=8)
    ...
    def test_determine_strategy_empty_text(self):
        ...
        strategy, chunks, workers = determine_strategy("", language="en", max_workers=4)
        assert strategy == "direct generation"
        assert chunks == 1
        assert workers == 1
        ...
        with patch("TTS_ka.ultra_fast.generate_audio_ultra_fast", side_effect=mock_generate_audio):
            with patch("TTS_ka.ultra_fast.ProgressTracker") as mock_tracker:
                ...
                with patch("TTS_ka.ultra_fast.concatenate_audio_files") as mock_concat:
                    ...
                    result = await process_chunks_parallel(chunks, output_path, "en", max_workers=2)
    ...
    async def test_process_chunks_parallel_single_chunk(self, temp_dir):
        ...
        with patch("TTS_ka.ultra_fast.generate_audio_ultra_fast", side_effect=mock_generate_audio):
            result = await process_chunks_parallel(chunks, output_path, "en", max_workers=1)
    ...
    async def test_generate_tts_turbo_direct(self, sample_text, temp_dir):
        ...
        with patch("TTS_ka.ultra_fast.generate_audio_ultra_fast", side_effect=mock_generate_audio):
            with patch("TTS_ka.ultra_fast.determine_strategy") as mock_strategy:
                ...
                result = await generate_tts_turbo(sample_text, output_path, "en", max_workers=4)
    ...
    @pytest.mark.parametrize(
        "chunk_count,max_workers,expected_min", [(1, 4, 1), (5, 4, 4), (10, 8, 6), (20, 6, 6)]
    )
