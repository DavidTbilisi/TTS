...
def _resolve_output_path(raw: Optional[str], force: bool = False,
    ...
    """
    if raw is None:
        # Still need to make sure parent dirs exist for config-derived paths.
...
For comprehensive help with examples: %(prog)s --help-full
    ...
    parser.add_argument(
        ...
        "--output",
        default=None,
        help="Output MP3 path (default: data.mp3). Parent dirs are created.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"TTS_ka {_get_version()}",
    )

    # Layer in config-file + env-var defaults BEFORE parse so CLI still wins.
    settings = load_settings()
    layered: dict = {}
    if settings.get("lang"):
        layered["lang"] = settings["lang"]
    if settings.get("chunk_seconds") is not None:
        layered["chunk_seconds"] = settings["chunk_seconds"]
    if settings.get("parallel") is not None:
        layered["parallel"] = settings["parallel"]
    # auto_play / turbo are inverted into --no-* flags
    if settings.get("auto_play") is False:
        layered["no_play"] = True
    if settings.get("turbo") is False:
        layered["no_turbo"] = True
    if settings.get("player"):
        layered["player"] = settings["player"]
    # output_dir is a special case: prefix data.mp3 in that dir when no --output given.
    if settings.get("output"):
        layered["output"] = settings["output"]
    elif settings.get("output_dir"):
        layered["output"] = os.path.join(settings["output_dir"], "data.mp3")
    if layered:
    ...
    if args.list_voices:
        ...
        # When --list-voices is used with no explicit --lang, show all.
    ...
    text = get_input_text(args.text) if args.text != "-" else args.text
    # If args.text was already overwritten with stdin contents, treat it as direct text
    if not text:
        if args.json:
    ...
    output_path = _resolve_output_path(
        ...
        force=args.force,
        fallback=args.output or "data.mp3",
    )
    ...
    async def run_generation() -> None:
        ...
        try:
            ...
            elif args.chunk_seconds > 0 or len(text.split()) > 200 or args.stream:
