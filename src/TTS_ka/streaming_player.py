...
class StreamingAudioPlayer:
    ...
    def _playback_worker_windows(self) -> None:
        ...
        while True:
            ...
            if not first_played:
                try:
                    if use_vlc:
                        # Ensure type for mypy: vlc_player must be str here
                        assert isinstance(vlc_player, str)
        ...
        if "vlc" in player:
            ...
            if chunks:
                ...
                try:
                    ...
                    self.process = subprocess.Popen(
                        cmd + chunks, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
                    )
                except Exception as e:
    ...
    def _find_streaming_player(self) -> Optional[str]:
        ...
        # Prefer mpv on Unix, then vlc
        players = ["mpv", "vlc", "ffplay", "mplayer"]

        for player in players:
            try:
                if sys.platform.startswith("win"):
                    # Windows: check PATH only (use 'where')
                    result = subprocess.run(["where", player], capture_output=True, timeout=1)
                    if result.returncode == 0:
                        return player
                    # Do not probe filesystem paths when PATH check fails
                    continue
                else:
                    # Unix: use which
...
async def play_audio_streaming(
    chunks: List[str], merge_first: bool = True, show_gui: bool = True
